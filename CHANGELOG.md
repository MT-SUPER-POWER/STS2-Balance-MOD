...
- 可以自主调控选择开启和关闭的选项区域
- 设置界面：可以自主调控选择开启和关闭的平衡调整内容，调整后自动开启重启
...
- 设置：修改开关后在最后操作的选项下方提示「重启游戏」，无需滚回页面顶部；撤销全部改动后隐藏提示。
- 设置与图鉴：修复被禁用的自定义卡牌与遗物在重启后依然会显示在主菜单「卡牌图鉴」与「遗物图鉴」中的问题，实现所见即所得的完全隐形（SETTINGS-COMPENDIUM-VISIBILITY-01）。
- 修复死灵书时间一直扣血拿不到的问题
