...
- 设置/遗物：新增方舟联动遗物「精致的玩偶」与「女巫形态」可配置开关 (SETTINGS-DELICATE-DOLL-01)
  - 在 RitsuLib 设置菜单提供「启用方舟联动：精致的玩偶（女巫形态）」开关（默认开启）。
  - 关闭时，先古之民 Tanx 事件恢复原版纯净遗物候选池，同时从相关卡池与遗物池中彻底过滤精致的玩偶、女巫形态及巫术衍生卡牌。
  - 在 `README.md` 与 `README_EN.md` 的「Mod 设置」表格及相关事件/遗物手册中补全设置项与效果说明。
- 卡牌/能力：先古卡联动能力「女巫形态」（Witch Form）与专属减益「巫术虚弱」（Sorcery Weak）数值平衡调整 (POWER-WITCH-FORM-01)
  - **女巫形态**：移除己方回合开始额外抽牌（由 **2 张** 下调为 **0 张**，不再额外抽牌）；保留转化手牌中至多 2 张牌为升级版「巫术打击+」与「巫术防御+」的核心转化机制。
  - **巫术虚弱**：造成的攻击伤害减免由 **50%** 下调为 **40%**（伤害倍率从 `0.5` 调整为 `0.6`）；攻击后掉 1 层的掉层机制与层数逻辑保持不变。
  - **巫术打击+ / 巫术防御+**：保持 0 费、9 伤 / 8 甲、打出抽 1 张牌及消耗属性不变；**巫术易伤** 受到攻击伤害 +75%（倍率 `1.75`）保持不变。

## v0.3.7

### Added

- 遗物：新增明日方舟巫恋——先古遗物「精致的玩偶」与先古卡「女巫形态」接入先古之民 Tanx 事件。
- 事件：新增 1 代经典回归事件「竞技场」（Colosseum）——注入至第三幕（Glory 荣耀之都）。完整还原两阶段角斗机制：(EVENT-COLOSSEUM-01)

### Changed

- 遗物：增益研究者事件遗物「突变之力」（Mutagenic Strength）力量平缓衰减重做——战斗开始时获得 3 点力量并附加 3 层专属 Debuff「突变衰退」（Mutagenic Decay），每回合结束流失 1 点力量直到 3 点扣完自动移除（可与人工制品 Artifact 正常联动抵挡流失保留永久力量）。同时为该 Debuff 设计并引入了基于 J.A.X. 折断针筒的专属透明图标。(RELIC-MUTAGENIC-01)
- 怪物/卡牌：永世沙漏（Aeonglass）状态牌「凋萎」（Wither）机制调整——对 FakeUpgrade 打 Patch，卡牌在战斗中每累计升级 2 次，打出消耗的费用 +1（初始 1 费，2 级 2 费，4 级 3 费，依次递增；新生成的凋萎卡牌通过自动补全升级次数同步对应费用）。(CARD-WITHER-01)
...
- 基础设施/CI：清理 CI 中无效的测试步骤与触发路径，工作流精简为 `Code Check`（格式检查与编译构建）；同步清理工程配置与说明文档中的失效单元测试描述。
