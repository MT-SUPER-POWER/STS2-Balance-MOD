...
- 设置：新增 78 项分区开关、当前生效与待重启筛选；先古之民与普通遗物独立分区，保留旧配置选择。
- 设置：新增主菜单「保存并重启游戏」入口，配置统一在下次启动应用。
- 联机：在原版握手成功后补充有效玩法配置检查；不一致沿用入房失败流程显示专属差异提示，其他错误仍交原版处理。

### Changed

- 设置界面：内容分区默认折叠、侧栏使用短标题；移除分区、角色及全局全开 / 全关按钮，展开后逐项调整；筛选使用下拉选择，重启提示按需显示。

- 新增内容通过获取入口控制启停，保留旧存档模型；关闭原版调整时同时恢复对应原版说明。
- 文档：补充贡献指南中的 props 路径、DLL / Stub 用途、构建方式与目录分层。
...
- 遗物：先古遗物「精致的玩偶」（Delicate Doll）拾起时增加诅咒代价，永久失去当前生命上限的 50%。（RELIC-DELICATE-DOLL-01）
