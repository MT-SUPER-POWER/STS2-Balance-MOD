...
- 改造师配图：替换为用户最终指定的图片，直接同步原图至母版、预览与游戏资源。(EVENT-ART-AUGMENTER-06)

- 竞技场配图：修正右侧蓝袍人物的侧脸，重画底部观众的排座、遮挡与远近比例。(EVENT-ART-COLOSSEUM-05)

- 诅咒书本配图：扶正石台并修正透视，减少背景装饰与烟雾细节。(EVENT-ART-TOME-04)

- 竞技场配图：为右侧前景观众补画蓝色兜帽，修正下半脸轮廓衔接。(EVENT-ART-COLOSSEUM-03)

- 牧师配图：简化洞窟背景，移除浮石、符文与建筑装饰，保留角色及双手光效。(EVENT-ART-CLERIC-02)

- 事件配图：使用 imagegen 逐张重画 10 张自制图，按原版约 `2.13:1` 重新构图并补画背景，保留主题与画风；规范到 `3440×1616` 并同步 `Assets/events/` 预览，旧素材另行备份。(EVENT-ART-RATIO-01)

### Added

- 联动内容：新增明日方舟联动——先古遗物「精致的玩偶」与先古卡「女巫形态」接入先古之民 Tanx 事件。(CROSSOVER-DELICATE-DOLL-01)

```mermaid
graph LR
    A["先古之民 Tanx 事件"] -->|设置开启时随机 3 选 1| B["先古遗物：精致的玩偶"]
    B -->|拾起触发| C["获得升级版先古卡：女巫形态+"]
    B -->|第 1 回合开始| D["全场施加 2 层易伤 + 2 层虚弱"]
    C -->|卡牌 HoverTips| G["预览展示巫术打击+与巫术防御+"]
    C -->|战斗中打出| E["获得女巫形态能力 & 结束回合"]
    E -->|每回合开始| F["额外抽 2 张牌 & 蜕变手牌为巫术打击+与巫术防御+"]
```

| 联动元素 | 归属类型 | 稀有度 / 费用 | 详细效果 |
| :--- | :---: | :---: | :--- |
| **精致的玩偶 [Delicate Doll]** | 先古遗物 | 先古 (Ancient) | 拾起时将一张已升级的「女巫形态+」加入牌组。第 1 回合开始时，对所有敌人施加 2 层易伤与 2 层虚弱（遗物悬浮预览仅展示「女巫形态+」）。 |
| **女巫形态 [Witch Form]** | 先古卡 (能力) | 3 费 (升级固有) | 获得 1 层「女巫形态」能力，结束你的回合；卡牌悬浮预览附带展示「巫术打击+」与「巫术防御+」卡牌效果。 |
| **女巫形态能力 [Witch Form Power]** | 玩家正面能力 | Buff (单层) | 回合开始时多抽 2 张牌，并从手牌中选择 2 张牌分别蜕变为升级后的「巫术打击+」与「巫术防御+」。 |
| **Tanx 事件候选池扩展** | 事件 Patch | - | Harmony Patch 拦截 Tanx 选项池，当 Mod 设置开关开启时加入精致的玩偶参与随机抽取。 |
| **可控开关** | Mod 设置项 | 默认开启 (true) | 允许玩家在 Mod 设置中自主开启或关闭该联动内容。 |
...
- 卡牌平衡：将铁甲战士卡牌「跃跃欲试」（Expect a Fight）中文译名调整为「且战且退」（CARD-15）。
...
- 遗物：日晷每累计洗牌 3 次获得的能量由 2 点提高至 3 点，触发频率与跨战斗计数保持不变（SUNDIAL-01）。
- 遗物：坚固钳子的格挡保留上限由 20 点下调至 15 点，并同步更新描述动态数值（STURDY-CLAMP-01）。
...
- 事件：心灵绽放第一战结算后恢复事件，玩家可带着 50 金币与稀有遗物离开，或接受不预览 Boss 与强化的未知第二战；点击后等概率抽取守护者、六火亡魂或史莱姆老大，并分别从耐久池（生命 +25% / 10 层覆甲 / 10 层再生）和威胁池（定制力量 / 1 层仪式）各抽取一项。胜利额外获得 100 金币、稀有遗物和罕见遗物，并保留普通怪物房奖励；强化只作用于开场 Boss，史莱姆分裂体不继承（MIND-BLOOM-02）。
...
- 事件：修复心灵绽放第一战结束后清空战斗回放、第二战却未重新调用 `RecordInitialState`，导致首回合 checksum 抛出异常、结束回合控件消失且战斗卡死的问题；同时将金币/遗物过滤器收窄到第一战包装遭遇，使第二战按设计保留普通怪物房金币、卡牌与药水概率（MIND-BLOOM-02）。
- 事件：修复心灵绽放第一战胜利后退出并读档会重建初始页面、从而可重复挑战并领取奖励的问题；事件现在根据当前地图点已保存的 `MindBloomBossEncounter` 胜利历史恢复战间抉择页（MIND-BLOOM-04）。
- 图鉴：修复心灵绽放第一战包装遭遇将密林与暗港全部原版一层 Boss、随从错误归入「事件」分组的问题。
