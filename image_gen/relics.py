...
def relic_output_name(src: Path) -> str:
    """遗物小图/大图输出文件名（PascalCase 驼峰）。"""
    return f"{src.stem}.png"


def outline_output_name(src: Path) -> str:
    """遗物轮廓图输出文件名（PascalCase 驼峰）。"""
    stem = src.stem
    for suffix in ("_outlines", "_outline", "Outlines", "Outline"):
...
def collect_sources(input_dir: Path, names: list[str] | None, *, label: str, required: bool = True, ) -> list[Path]:
    """收集待处理的源文件列表。"""
    if names:
        files = []
        for name in names:
...
def main() -> None:
    ...
    out_outline_dir = args.output / "outlines"
    ...
    if outline_sources:
        ...
        print(f"轮廓输出: {out_outline_dir}")
        print(f"共 {len(outline_sources)} 张\n")

        for src in outline_sources:
            process_outline(
                src,
                out_outline_dir / outline_output_name(src),
            )
