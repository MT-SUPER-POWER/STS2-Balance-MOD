...
在 RitsuLib 的 Mod 设置页面按分区调整，默认全部开启；已有三个开关的选择会保留。内容分区默认折叠，展开后逐项开关，不提供全开 / 全关。侧栏保留简短分区名，条目说明显示当前状态。完整条目见 [分区开关清单](docs/settings-catalog-proposal.md)。

| 分区 | 控制内容 |
| --- | --- |
| 新增卡牌 / 新增普通遗物 | 每张卡牌、每件普通遗物独立开关 |
| 回归事件 | 11 个事件独立开关；专属奖励和配套机制跟随事件 |
| 原版卡牌 / 普通遗物 / 事件调整 | 逐项恢复或启用调整，三个事件的离开选项分别控制 |
| 先古之民调整 | 独立于普通遗物，按先古之民分组；玩偶联动归探克斯，咖啡杯等归达尔夫 |
| 怪物 / 通用规则 | 凋萎、感染棱柱、商店删牌与诅咒钥匙单人跳过宝箱 |

配置保存后统一**重启游戏生效**。 修改开关后，刚操作的选项下方会显示「重启游戏」按钮，可继续调整后统一重启；撤销全部待生效改动后提示消失。列表通过下拉选择「全部 / 当前生效 / 待重启」，区分已启用、已关闭和待生效的选择；主菜单设置页提供「保存并重启游戏」，对局或房间内请先退出到主菜单。关闭新增内容会阻止后续常规获取并在图鉴中彻底隐形，旧存档已经持有的内容仍保留，不撤销已经发生的永久变化。

联机入房时检查双方**本次启动生效的玩法配置**。不一致会沿用入房失败流程，提示「Mod 配置不一致」和房主 / 本人的差异；不会自动同步配置。其他版本、连接、超时等错误保留原版提示。

开发环境的路径、DLL 与目录说明见 [贡献指南](docs/contribute.md)。
...
| **凋萎 [Wither]** | <img src="Assets/map/aeonglass_boss.png" width="22" height="22" title="永世沙漏 (Aeonglass)"> | 状态 | 不可打出，回合结束受伤害，随沙漏成长 | 改为 **1 费可打出**，打出后消耗；且**每累计 2 次升级打出费用 +1**（保留成长机制） |
...
| <img src="Assets/relics/royal_stamp.png" width="22" height="22" valign="middle"> **王室印章 [Royal Stamp]** | <img src="Assets/profile/merchant.png" width="22" height="22" title="商人 (Merchant)"> | 拾起时只能从牌组中选择一张攻击牌或技能牌附魔【王室认证】（固有+保留） | 放宽附魔类型约束，**能力牌**亦可被附魔【王室认证】 |
