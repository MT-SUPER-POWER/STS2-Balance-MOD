...
| 卡牌 | 角色 | 类型 | 原版 | MOD 改后 |
|------|------|:----:|------|------|
| **挽歌** | 骨妹 | 能力 | 升级后召唤次数 +1，消耗，灵魂+ | 升级后**额外追加保留词条** |
| **刀舞** | 猎人 | 技能 | 白卡（普通），打出后消耗 | 删除消耗词条，稀有度升为**蓝卡（罕见）**，可多次复用 |
| **杂技** | 猎人 | 技能 | 蓝卡（罕见），入手门槛较高 | 稀有度降为**白卡（普通）**，提升入手概率 |
| **认知偏差** | 猎人 | 能力 | 每回合 -1 聚焦，永久持续 | 聚焦在对应回合后自动消失，不再扣除超出提升点数的聚焦 |
| **多重释放** | 鸡煲 | 技能 | 升级后 X+1 次释放 | 升级改为仅追加**保留**词条（取消原版 X+1）|
| **放松** | 佩尔 | 技能 | 格挡 15（升级 17）| 格挡调整为 **18（升级 20）** |
| **幽魂形态** | 佩尔 | 技能 | 获得无实体后每回合累积减敏捷负面 | 打出时只施加无实体，**彻底删除减敏捷负面效果** |
| **袖里乾坤** | 猎人 | 技能 | 每次打出后减少费用，定位与刀舞冲突 | 从猎人卡池中**完全移除** |
| **燃料** | 鸡煲 | 技能 | 将所有状态牌转换，2 费 | 改为 **1 费**，获得 1 点能量并**抽 1（升级：2）张牌** |
| **能量汲取** | 骨妹 | 攻击 | 造成 10/12 伤害，升级后随机升级弃牌堆 3 张 | 伤害降为 **6/8**；基础随机升级弃牌堆 **2 张**，升级后改为升级弃牌堆**全部可升级牌** |
| **吸引仇恨** | 骨妹 | 技能 | 升级后原版数值 | 升级后调整为：**召唤骨头 6 个，格挡 9 点** |
| **凋零** | 永世沙漏 | 状态 | 不可打出，进入消耗堆 | 改为 **1 费可打出**，打出后消耗 |
| **辉光** | 佩尔 | 技能 | 获得星尘，下回合额外抽牌 | 改为**当回合立即抽 2 张**，升级额外多获得 1 星尘 |
| **放血** | 战士 | 技能 | 蓝卡（罕见）（v1.0.9 修改） | 稀有度改回**白卡（普通）** |
| **创世之柱** | 无色 | 能力 | 每回合第一次生成卡牌时获得 5/8 点格挡 | 改为每生成一张卡牌均获得 **3/4 点格挡**（取消每回合首次限制） |
| **残酷** | 战士 | 能力 | 升级后原版数值，下放为战士卡牌 | 从战士卡池中**完全移除**（由“进化”卡牌替代） |
| **探寻** | 无色 | 任务 | 进入 5 个 ? 房间后转化为「丰饶」 | 调整为进入 **4 个 ? 房间**后转化为「丰饶」 |
...
|------|------|:----:|:------:|:----:|------|
| **死亡收割** | 战士 | 攻击 | 稀有 | 2 | 消耗。对所有敌人造成 **4 / 6** 点伤害，并回复等量于实际造成的非格挡伤害的生命值。 |
| **硬撑** | 战士 | 技能 | 罕见 | 1 | 获得 **15 / 20** 点格挡，将 2 张伤口加入手牌。 |
| **全神贯注** | 猎人 | 技能 | 罕见 | 0 | 丢弃 **3 / 2** 张牌，获得 2 点能量。 |
| **电动力学** | 机器人 | 能力 | 稀有 | 2 | 召唤 **2 / 3** 个闪电球，且闪电球改为攻击所有敌人。 |

### 新增卡牌

| 卡牌 | 角色 | 类型 | 稀有度 | 费用 | 效果（基础 / 升级） |
|------|------|:----:|:------:|:----:|------|
| **比试** | 骨妹 | 攻击 | 罕见 | 2 | 消耗。玩家对单个敌人造成 **8** 点伤害，奥斯提造成 **7 / 9** 点伤害；实际造成非格挡伤害较少的一方回复 **4 / 6** 点生命。 |
| **猛撞** | 骨妹 | 攻击 | 普通 | 2 | 奥斯提失去 **6 / 5** 点生命，对所有敌人造成 **20 / 26** 点伤害；奥斯提生命不足时无法触发效果。 |
| **步步为营** | 猎人 | 技能 | 稀有 | X | 消耗。接下来 X（升级：X+1）回合，每回合多抽 1 张牌并多获得 1 点能量。升级后额外获得保留词条。 |
| **进化** | 战士 | 能力 | 罕见 | 1 | 每当你抽到一张状态牌，抽 **1 / 2** 张牌。 |
...
| 事件名称 | 先行条件 | 详情 |
| :--- | :--- | :--- |
| **老乞丐事件** | 所有玩家金币 ≥ 75 | 给金币后切换为牧师删牌。 |
| **诅咒书本** | Act 2，且牌组无对应书籍遗物 | 可获得死灵之书、尼利的宝典、英雄宝典。 |
| **红面具** | Act 2，层数 ≥ 23，且无人持有红面具 | 可选择交金或与红面具三人帮战斗获取红面具。 |
| **J.A.X.** | Act 2，且所有玩家牌组中可移除牌 ≥ 2 张 | 获得 J.A.X. 卡牌 / 变2张牌 / 获得突变之力遗物。 |
| **神圣泉水** | 所有玩家牌组中存在可移除的诅咒牌 | 移除牌组中全部可移除诅咒（删除原版的伤害副作用）。 |
| **牧师** | 所有玩家金币 ≥ 35 | 提供付钱选择治疗（25% 最大 HP）/ 删牌选项（75 金）。 |
| **心灵绽放** | Act 3 | 1. **战斗**——随机召唤第一幕的 Boss 进行决战，胜利获得 50 金 + 稀有遗物。<br>2. **升级**——升级牌组中所有可升级的牌，并获得「绽放印记」遗物。<br>3. **宝库**（层数 < 41，多人 < 38）获得 999 金 + 牌组加入 2 张「凡庸」。|
| **大转盘** | - | 自定义转盘小游戏，随机获得金/遗物/治疗/诅咒/删牌/受伤。 |
| **红面具大人之墓** | Act 3，且无人持有红面具 | 可献上全部金币获得红面具，或（持有红面具时）收获 222 金。 |
...
| 遗物 | 类型 / 编号 | 描述 |
| :--- | :--- | :--- |
| **日晷** | 商店 | 每将抽牌堆洗牌 3 次（跨战斗保留计数），获得 2 点能量。 |
...
| **诅咒钥匙** | 先古 / RELIC-01 | 每回合 +1 费用，每次打开宝箱获得一张随机诅咒。**仅限单人模式出现，单人模式下可通过右下角的"跳过宝箱"按钮（ProceedButton）直接离开，同时规避诅咒。** |
| **矮人铁砧** | 商店 | 拾起时为 3 张牌附加"锻造"附魔，被附魔的牌费用永久 -1（最低 0 费）。 |
| **袖箭** | 罕见 | 猎人专属。费用为 0 的攻击牌额外造成 4 点伤害。 |
| **悬浮风筝** | 普通 | 猎人专属。你在每回合第一次弃牌时，获得 1 点能量。 |
| **灵魂契约** | 先古 | 拾起时扣除最大生命上限 10%。选择牌组中的 1 张有消耗的牌，永久去除其消耗属性。 |
...
| 遗物 | 类型 | 原版 | MOD 改后 |
| :--- | :--- | :--- | :--- |
| **坚固钳子** | 稀有 | 保留 10 护甲 | 保留 **20 护甲** |
| **活雾** | 先古 | 删除 3 张牌 | 删除 **4 张牌** |
| **红面具** | 事件 | 在一般共享遗物池中 | 从一般共享遗物池**移除**，只通过红面具相关事件获得 |
