...
在 RitsuLib 的 Mod 设置页面按分区调整，默认全部开启；已有三个开关的选择会保留。每项可以独立开关，分区提供全开 / 全关，并显示全部开启、部分开启或全部关闭。完整条目见 [分区开关清单](docs/settings-catalog-proposal.md)。
...
配置保存后统一**重启游戏生效**。列表支持「全部 / 当前生效 / 待重启」，区分已启用、已关闭和待生效的选择；主菜单设置页提供「保存并重启游戏」，对局或房间内请先退出到主菜单。关闭新增内容会阻止后续常规获取，旧存档已经持有的内容仍保留，不撤销已经发生的永久变化。
