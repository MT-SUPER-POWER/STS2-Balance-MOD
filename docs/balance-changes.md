...
- [x] **MIND-BLOOM-04** — 防止读档重复领取第一战奖励
  - 第一战胜利状态改由当前地图点已经持久化的房间历史判断，只有 `MindBloomBossEncounter` 存在有效胜利回合数时才视为完成。
  - 第一战完成后退出并读档会直接恢复“离开”与“未知第二战”的战间抉择页，不再重新显示第一阶段选项或重复发放第一战奖励。
  - 新事件仍显示原始三个选项；第一战入口、第二战随机 Boss、强化及奖励规则均保持不变，并通过静态回归检查与 `dotnet build --no-restore` 验证。
