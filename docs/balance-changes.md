...
- [x] **CARD-14** — 火箭飞拳 (Rocket Punch) 0 费效果修改
  - 目前: 触发生成状态牌后，火箭飞拳降为 0 费的效果可对多张飞拳生效且会持续为 0 费。
  - 目标: 无论生成多少张状态牌，仅赋予“下 1 张打出的【火箭飞拳】0 费”的单次消耗标志，不能按生成张数或多张飞拳多次/无限累加。打出任意 1 张【火箭飞拳】后立即消耗该标志，手牌及牌组中其余【火箭飞拳】立刻恢复 2 费；只有后续再次生成状态牌后才能重新获得 1 次 0 费额度。
  - 数值: 基础 2 费；生成状态牌时仅单次打出的第 1 张【火箭飞拳】降为 0 费。
  - 备注: 需重构 `RocketPunchRollbackPatch.cs`，改为玩家层级/战斗全局标志控制或同步刷新同名卡牌费用。
