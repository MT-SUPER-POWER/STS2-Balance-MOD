...
- [x] SHOP-01 — A6 难度以上删牌价格调整为 25 金币（低进阶 50+25/次，高进阶 75+25/次）
...
- [x] RELIC-09 — 遗物-活雾（瓦库） 删3改为删4
- [ ] RELIC-10 — 商店遗物-矮人铁砧: 你可以选择一张 攻击 / 技能牌在火堆处不断升级
...
- [x] STS1-EVENT-02 — 移植红面具事件（Masked Bandits，含红面具帮战斗；需要红面具帮战斗遭遇）
- [x] STS1-EVENT-03 — 移植增益研究者事件（J.A.X. 事件）
- [x] STS1-EVENT-04 — 移植神圣泉水事件（删除诅咒）
- [x] STS1-EVENT-05 — 移植牧师事件（治疗 / 删牌）
- [x] STS1-EVENT-06 — 移植心灵绽放事件（已实现升级全牌+绽放印记、999 金币、回复分支；打一层 Boss 分支从本局密林/暗港中玩家已遭遇过的原版 Boss 随机选取）
...
- [x] BUG-08 — 红面具直接从道具删除了，应该改为 Event 遗物才对
...
- [x] BUG-11 — 强盗事件熊是直接减敏捷，写错了，是给一层脆弱
- [ ] BUG-12 — 面具强盗事件注册的遭遇战，事件版本的遇不到了

## FEATURE 列表

- [ ] FEATURE-01 — 游戏里没有宝箱跳过的选项，那么多人完全不知道如何做，所以**诅咒钥匙**现在基本就是拿了稳定三诅咒的
- [ ] FEATURE-02 — 时间老头在多人战斗过于恐怖，如何平衡设计是一个问题
- [ ] FEATURE-03 — 时间吞噬者 BGM 体系改女王的，取消现有的注入机制
