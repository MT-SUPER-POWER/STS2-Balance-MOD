...
- [x] **SUNDIAL-01** — 加强日晷的触发回能
  - 每累计将抽牌堆洗牌 3 次获得的能量由 2 点提高至 3 点。
  - 商店遗物稀有度、跨战斗/读档计数、持有者判定、计数器显示与激活动画保持不变。

- [x] **STURDY-CLAMP-01** — 将坚固钳子的格挡保留上限下调至 15
  - 实际格挡保留上限与描述动态变量均由 20 点下调至 15 点。
  - 高于 15 时只清除超出部分，等于或低于 15 时保留当前值且不补齐；其他触发规则保持不变。
