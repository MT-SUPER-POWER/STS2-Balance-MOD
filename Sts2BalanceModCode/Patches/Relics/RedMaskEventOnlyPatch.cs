...
{
  [HarmonyPostfix]
  public static RelicModel[] Postfix(RelicModel[] __result) => [.. __result.Where(r => r is not RedMask)];
}
...
{
  [HarmonyPostfix]
  public static IEnumerable<RelicModel> Postfix(IEnumerable<RelicModel> __result) => __result.Append(ModelDb.Relic<RedMask>());
}
