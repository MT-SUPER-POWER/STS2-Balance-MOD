...
public static class InfestedPrismPatch
{
  ...
  [HarmonyPrefix]
  public static bool AfterAddedToRoomPrefix(ref Task __result)
  {
  ...
  public static bool GenerateMoveStateMachinePrefix(InfestedPrism __instance, ref MonsterMoveStateMachine __result)
  {
    ...
    var move1 = new MoveState("LIGHT_ATTACK_MOVE", targets => LightAttackMove(__instance, targets), new SingleAttackIntent(lightDamage), new DebuffIntent());
    var move2 = new MoveState("HEAVY_ATTACK_MOVE", _ => HeavyAttackMove(__instance), new SingleAttackIntent(heavyDamage), new DefendIntent());
    var move3 = new MoveState("MULTI_ATTACK_MOVE", _ => MultiAttackMove(__instance), new MultiAttackIntent(multiDamage, 3));
    var move4 = new MoveState("FORTIFY_MOVE", _ => FortifyMove(__instance), new DefendIntent(), new BuffIntent());
    ...
    var states = new List<MonsterState> { move1, move2, move3, move4 };
  ...
  private static async Task HeavyAttackMove(InfestedPrism prism)
  {
    int heavyDamage = AscensionHelper.GetValueIfAscension(AscensionLevel.DeadlyEnemies, 22, 20);
  ...
  private static async Task MultiAttackMove(InfestedPrism prism)
  {
    int multiDamage = AscensionHelper.GetValueIfAscension(AscensionLevel.DeadlyEnemies, 7, 6);
  ...
  private static async Task FortifyMove(InfestedPrism prism)
  {
