...
public sealed class PeacePipe : BalanceRestSiteRelicTemplate
{
  ...
  protected override bool CanAddRestSiteOption(Player player, ICollection<RestSiteOption> options) =>
    ...
    options.All(static option => option is not PeacePipeRestSiteOption);

  protected override RestSiteOption CreateRestSiteOption(Player player) => new PeacePipeRestSiteOption(player);
}
