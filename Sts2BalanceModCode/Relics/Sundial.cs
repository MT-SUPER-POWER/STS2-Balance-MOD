...
/// </summary>
[RegisterRelic(typeof(SharedRelicPool), FullPublicEntry = "STS2_BALANCEMOD_SUNDIAL")]
public sealed class Sundial : BalanceRelicTemplate
{
  // NOTE: 这两个 key 与 relics.json 中的 {Shuffles} 和 {Energy} 占位符一一对应，
  //       用于本地化文本的动态数值替换
  private const string _shufflesKey = "Shuffles";
  ...
  public override int DisplayAmount
  {
    ...
    {
      if (IsActivating)
        return base.DynamicVars[_shufflesKey].IntValue;
  ...
  [
    new DynamicVar(_shufflesKey, 3m),   // 需要 3 次洗牌触发回能
  ...
  public override async Task AfterShuffle(PlayerChoiceContext choiceContext, Player shuffler)
  {
    ...
    int requiredShuffles = base.DynamicVars[_shufflesKey].IntValue;
    ...
    if (ShufflesSeen == 0)
    {
      ...
      await PlayerCmd.GainEnergy(
        base.DynamicVars[_energyKey].BaseValue,
        shuffler
      );
    }
  }
  ...
  private async Task DoActivateVisuals()
  {
    ...
    // 触发动画结束后，根据当前计数恢复状态
    base.Status = ShufflesSeen == base.DynamicVars[_shufflesKey].IntValue - 1
      ? RelicStatus.Active
      : RelicStatus.Normal;
  }
