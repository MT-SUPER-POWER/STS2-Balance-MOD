...
public sealed class TimeEater : BalanceMonsterTemplate
{
  ...
  private static int BaseInitialHp => AscensionHelper.GetValueIfAscension(AscensionLevel.ToughEnemies, 480, 456);
  ...
  private static int ReverberateDamage => AscensionHelper.GetValueIfAscension(AscensionLevel.DeadlyEnemies, 8, 7);

  private static int HeadSlamDamage => AscensionHelper.GetValueIfAscension(AscensionLevel.DeadlyEnemies, 32, 26);

  private static bool HasDoubleBossAscension => AscensionHelper.HasAscension(AscensionLevel.DoubleBoss);
  ...
  private async Task ScaleHpForMultiplayer()
  {
    ...
    int previousMaxHp = Creature.MaxHp;
    await CreatureCmd.SetMaxHp(Creature, ScaledInitialHp);

    int gainedMaxHp = ScaledInitialHp - previousMaxHp;
    if (gainedMaxHp > 0)
    {
  ...
  protected override MonsterMoveStateMachine GenerateMoveStateMachine()
  {
    ...
    AbstractIntent[] headSlamIntents = HasDoubleBossAscension
      ? [new SingleAttackIntent(global::Sts2BalanceMod.Sts2BalanceModCode.Monsters.TimeEater.HeadSlamDamage), new DebuffIntent(), new StatusIntent(SlimedCount)]
      : [new SingleAttackIntent(global::Sts2BalanceMod.Sts2BalanceModCode.Monsters.TimeEater.HeadSlamDamage), new DebuffIntent()];
  ...
  private async Task HasteMove(IReadOnlyList<Creature> targets)
  {
    ...
    decimal healAmount = Creature.MaxHp / 2M - Creature.CurrentHp;
    if (healAmount > 0M)
    {
