...
/// </summary>
public abstract class BalanceRestSiteOption(Player owner) : RestSiteOption(owner)
{
  ...
  /// 输出：可删除卡牌数量。
  /// </summary>
  protected static int GetRemovableCardCount(Player player) => PileType.Deck.GetPile(player).Cards.Count(static c => c.IsRemovable);
