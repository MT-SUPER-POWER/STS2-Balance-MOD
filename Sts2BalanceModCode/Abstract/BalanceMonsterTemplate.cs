...
public abstract class BalanceMonsterTemplate : ModMonsterTemplate
{
  ...
  {
    string locMoveId = moveStateId.EndsWith("_MOVE") ? moveStateId[..^5] : moveStateId;
  ...
  protected static bool LastMove(MonsterMoveStateMachine? stateMachine, string moveId)
  {
    ...
    List<MonsterState> log = stateMachine.StateLog;
  ...
  public override List<BestiaryMonsterMove> GenerateBestiaryMoveList(NCreatureVisuals? creatureVisuals)
  {
    ...
    Dictionary<string, MonsterState>? states = MoveStateMachine?.States;
    ...
    foreach (KeyValuePair<string, MonsterState> state in states)
    {
      string stateId = state.Key;
      MonsterState monsterState = state.Value;
      ...
      string moveId = stateId;
      LocString moveName = MoveTitle(moveId);
