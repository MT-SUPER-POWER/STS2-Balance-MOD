...
public sealed class Augmenter : BalanceEventTemplate
{
  ...
  private async Task Jax()
  {
    ...
    CardModel jax = owner.RunState.CreateCard(ModelDb.Card<Jax>(), owner);
    CardPileAddResult result = await CardPileCmd.Add(jax, PileType.Deck);
  ...
  private async Task Transform()
  {
    ...
    foreach (CardModel? card in selectedCards.ToList())
