...
/// </summary>
public sealed class NWheelSpinScreen : IDisposable
{
  // ─── 布局 ───
  ...
  private void BindEvents() => _minigame.Finished += OnMinigameFinished;

  private void UnbindEvents() => _minigame.Finished -= OnMinigameFinished;
  ...
  {
    Texture2D wheelTex = GD.Load<Texture2D>(WheelTexPath);
    ...
    Texture2D arrowTex = GD.Load<Texture2D>(ArrowTexPath);
    const float halfArrow = ArrowDisplaySize / 2f;
    ...
    Texture2D buttonTex = GD.Load<Texture2D>(ButtonTexPath);
    const float halfButton = ButtonDisplaySize / 2f;
  ...
  private void StartBounceIn()
  {
    ...
    _mainTween.TweenMethod(
      ...
      {
        const float halfWheel = WheelDisplaySize / 2f;
  ...
  private void StartSpinning()
  {
    ...
    const float spinEnd = SpinVelocity * SpinDuration;
  ...
  private void OnMinigameFinished() => DisposeInstance();
  ...
  private static float ElasticLerp(float from, float to, float t) => from + (to - from) * ElasticIn(t);
