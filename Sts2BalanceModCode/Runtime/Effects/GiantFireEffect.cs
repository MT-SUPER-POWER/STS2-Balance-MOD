...
public sealed class GiantFireEffect : NSts1Effect, IDisposable
{
  ...
  protected override void Initialize()
  {
    ...
    string flameName = Random.Shared.Next(3) switch
    {
      0 => "combat/flame4",
      1 => "combat/flame5",
      _ => "combat/flame6",
    ...
    LibGdxAtlas.TextureRegion? textureRegion = LibGdxAtlas.GetRegion(AtlasPath, flameName);
    ...
    float imageWidth = textureRegion.Value.Region.Size.X;
    float imageHeight = textureRegion.Value.Region.Size.Y;
    float x = (float)(Random.Shared.NextDouble() * ScreenWidth) - imageWidth / 2f;
    float y = ScreenHeight + (float)(Random.Shared.NextDouble() * 200.0 + 200.0) + imageHeight / 2f;
    ...
    _color = Colors.White;
    _color.A = 0f;
    float greenReduction = (float)(Random.Shared.NextDouble() * 0.5);
    _color.G -= greenReduction;
    _color.B -= greenReduction - (float)(Random.Shared.NextDouble() * 0.2);

    float rotation = (float)(Random.Shared.NextDouble() * 20.0 - 10.0);
  ...
  private static float Fade(float value) => Mathf.Clamp(value * value * value * (value * (value * 6f - 15f) + 10f), 0f, 1f);
