...
public static class LibGdxAtlas
{
  ...
  private static readonly Dictionary<string, AtlasData> AtlasCache = [];
  ...
  public static TextureRegion? GetRegion(string atlasPath, string regionName)
  {
    ...
    if (!atlasData.Regions.TryGetValue(regionName, out RegionData? region) || region.TexturePath == null)
      return null;

    Texture2D? texture = LoadTexture(region.TexturePath);
    return texture == null
      ? null
  ...
  {
    if (AtlasCache.TryGetValue(atlasPath, out AtlasData? cached))
      return cached;
  ...
  private static AtlasData ParseAtlasFile(string atlasPath)
  {
    ...
    string[] lines = file.GetAsText(skipCr: true).Split('\n');
  ...
  private static void ParseRegionProperty(string line, RegionData region)
  {
    ...
    string[] values = line[(colonIndex + 1)..]
