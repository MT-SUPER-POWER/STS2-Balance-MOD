...
public sealed class GhostlyFireEffect : NSts1Effect, IDisposable
{
  ...
  protected override void Initialize()
  {
    ...
    LibGdxAtlas.TextureRegion? textureRegion = LibGdxAtlas.GetRegion(AtlasPath, regionName);
  ...
  private void UpdateSprite()
  {
    ...
    float wobbleX = (float)GD.RandRange(0.95, 1.05);
    float wobbleY = (float)GD.RandRange(0.95, 1.05);
    _sprite.Scale = new Vector2(_scale * wobbleX, _scale * wobbleY);
    _sprite.Modulate = _color;
  }

  private static CanvasItemMaterial CreateAdditiveMaterial() => new() { BlendMode = CanvasItemMaterial.BlendModeEnum.Add };
