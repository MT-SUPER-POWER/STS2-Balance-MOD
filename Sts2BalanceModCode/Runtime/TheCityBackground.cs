...
public partial class TheCityBackground : NCombatBackground
{
  ...
  private TextureRect CreateTextureRect(string regionName, int zIndex)
  {
    ...
    LibGdxAtlas.RegionInfo? regionInfo = LibGdxAtlas.GetRegionData(AtlasPath, regionName);
    LibGdxAtlas.TextureRegion? region = LibGdxAtlas.GetRegion(AtlasPath, regionName);
  ...
  public void OnTreeEntered()
  {
    ...
    {
      Control sceneContainer = combatRoom.GetNodeOrNull<Control>("%CombatSceneContainer")
      ...
      Control allyContainer = combatRoom.GetNodeOrNull<Control>("%AllyContainer");
  ...
  private static void ReparentToContainer(TextureRect layer, Control container, int zIndex)
  {
    ...
    Vector2 globalPos = layer.GlobalPosition;
