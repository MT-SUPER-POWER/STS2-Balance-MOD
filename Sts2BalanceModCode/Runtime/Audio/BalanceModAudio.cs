...
public static class BalanceModAudio
{
  ...
  {
    AudioStream? stream = LoadAndLoopMusic(path);
    if (stream == null)
      return;

    StopMusicImmediate();

    _musicPlayer = CreateMusicPlayer(stream);
  ...
  public static void FadeIn(string path, float duration = 1.0f, float volumeDbOffset = 0f)
  {
    ...
    if (runNode != null)
    {
      ...
      float targetDb = CalculateVolumeDb(volumeDbOffset);
  ...
  /// </summary>
  public static void UpdateVolume() => ApplyVolume();
  ...
  private static AudioStreamPlayer CreateMusicPlayer(AudioStream stream)
  {
  ...
  {
    float bgmVolume = SaveManager.Instance?.SettingsSave?.VolumeBgm ?? 1f;
