...
public sealed class HexaghostVisuals : IDisposable
{
  ...
  {
    NCreatureVisuals visualsNode = _creatureNode.Visuals;
  ...
  {
    Texture2D texture = PreloadManager.Cache.GetTexture2D(path) ?? ResourceLoader.Load<Texture2D>(path);
    return new Sprite2D
    {
      Texture = texture,
      ZIndex = zIndex,
  ...
  private async Task UpdateLoop()
  {
    ...
    {
      SceneTree tree = _creatureNode.GetTree();
      if (tree == null)
      {
        await Task.Delay(50);
        continue;
  ...
  private void Update(float delta)
  {
    ...
    Vector2 parentGlobalPosition = _creatureNode.Visuals.GlobalPosition;
    foreach (HexaghostOrbVisual orb in _orbs)
      orb.Update(delta, parentGlobalPosition);
  }

  public void SetTargetRotationSpeed(float speed) => _targetRotationSpeed = speed;
